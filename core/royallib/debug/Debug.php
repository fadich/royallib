...
namespace royallib\debug;

use royallib\type\Mixed;
...
class Debug extends Interact
{
    ...
    public function startTimer($index = false)
    {
        ...
        return $this->setLog('timer "' . key(array_slice($this->_timer, -1, 1, true)) . '" started (' .
            $this->microdate . ')', 'timer');
    }
    ...
    public function dump($var)
    {
        if (is_array($var)) {
            $this->_log['dump'][] =
                'array(' . sizeof($var) . ') [' . (new Mixed($var))->implodeElements(", ", [], '=>')->value . ']';
    ...
    protected function log($data)
    {
        ...
        file_put_contents(
            self::logDir() . DIRECTORY_SEPARATOR . $this->getMicrodate('d-m-Y_H-m-s-') . '.txt',
